...
polo = poloniex('API_KEY_HERE','SECRET_HERE')

#openorders = polo.returnOpenOrders("BTC_NAUT")
x = 0
ibalance = polo.returnBalances()
initialBTC = dict((k, v) for k, v in ibalance.items() if k == 'BTC')
import time
starttime=time.time()
while True:
    balance = polo.returnBalances()
    values = dict((k, v) for k, v in balance.items() if k != 'BTC' and k != 'STR' and v!= '0.00000000')
